# Portfolio-Risk / Risk-Sizing 统一账户模型设计（设计阶段，尚未实现）

## 状态

**这是一份设计文档，不是实现任务。** 目的是回答"如何统一"，不是直接写代码。本文档产出后需要
用户审阅通过，才决定是否进入实现阶段（可能拆分为多个子任务）。
...
## 建议的下一步（供设计通过后参考，非本文档的实现范围）

1. 先由用户/团队决定第 2 条"手数为准 vs 权重为准"——这是唯一的产品性决策，其余问题的答案很大程度上
   取决于这一条。
2. 决定后，若采用"手数为准"，建议先在 `PortfolioCoordinator` 内新增一个跨品种共享的
   `PortfolioLedger`（暂定名）状态对象，把 `_run_per_symbol` 的"完全独立回测"改造为"共享保证金池的
   联合回测"——这是架构级改动，预计不是一个任务能完成，需要拆分（例如：先支持只读的"组合保证金占用
   估算报告"，验证估算准确后再接入真正的开仓判定阻断）。
3. 若采用"权重为准"，改造集中在 `positions.py` 的手数计算逻辑，让它接受组合层分配的名义敞口而非
   自主计算，风险相对小，但需要重新验证 A40 现有的手数/保证金等价性测试在"权重驱动手数"模式下是否
   仍然成立。
