"""A52 — Rollover-window tagging ``off`` full-engine equivalence regression.

Runs BacktestEngine end-to-end with ``rollover_stat_tagging="off"`` on
deterministic synthetic data.  The stored snapshot represents the legacy
baseline; any future change that alters the default/legacy path will produce a
diff and fail.
...
SNAPSHOT_PATH = Path(__file__).with_name("test_rollover_off_equivalence.snapshot.json")

DATASETS = [
    ("AP888", 15, 240, datetime(2024, 1, 2, 9, 0)),
    ("ZN888", 20, 180, datetime(2024, 3, 4, 9, 0)),
]
...
def test_rollover_off_full_engine_equivalence(synthetic_1m_bars, monkeypatch):
    """``rollover_stat_tagging="off"`` must reproduce the stored baseline exactly."""
    actual = {
        symbol: _run_symbol(
            monkeypatch,
    ...
    assert actual == baseline, "rollover_stat_tagging='off' output differs from stored baseline."
