"""A47 P8a — Exit-model legacy full-engine equivalence regression.

Runs BacktestEngine end-to-end with ``exit_model="legacy"`` on deterministic
...
SNAPSHOT_PATH = Path(__file__).with_name("test_exit_model_legacy_equivalence.snapshot.json")
...
def test_exit_model_legacy_full_engine_equivalence(synthetic_1m_bars, monkeypatch):
    """``exit_model="legacy"`` must reproduce the stored baseline exactly."""
    actual = {
        symbol: _run_symbol(
            monkeypatch,
    ...
    assert actual == baseline, "exit_model='legacy' output differs from stored baseline."
