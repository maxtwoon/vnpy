"""A45 P6 — Off-mode full-engine equivalence regression.

Runs BacktestEngine end-to-end with ``second_buy_mode="baseline"`` and
``atr_chop_filter="off"`` on deterministic synthetic data.  The stored
snapshot represents the legacy baseline; any future change that alters the
default/off path will produce a diff and fail the test.
...
SNAPSHOT_PATH = Path(__file__).with_name("test_second_buy_and_atr_off_equivalence.snapshot.json")
...
def test_second_buy_and_atr_off_full_engine_equivalence(synthetic_1m_bars, monkeypatch):
    """Default baseline/off settings must reproduce the stored baseline exactly."""
    actual = {
        symbol: _run_symbol(
            monkeypatch,
    ...
    assert actual == baseline, "second_buy_mode='baseline'/atr_chop_filter='off' output differs from stored baseline."
