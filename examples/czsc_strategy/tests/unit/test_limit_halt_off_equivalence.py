"""A51 — Limit-halt tagging ``off`` full-engine equivalence regression.

Runs BacktestEngine end-to-end with ``limit_halt_model="off"`` on deterministic
synthetic data.  The stored snapshot represents the legacy baseline; any future
change that alters the default/legacy path will produce a diff and fail.
...
SNAPSHOT_PATH = Path(__file__).with_name("test_limit_halt_off_equivalence.snapshot.json")
...
def test_limit_halt_off_full_engine_equivalence(synthetic_1m_bars, monkeypatch):
    """``limit_halt_model="off"`` must reproduce the stored baseline exactly."""
    actual = {
        symbol: _run_symbol(
            monkeypatch,
    ...
    assert actual == baseline, "limit_halt_model='off' output differs from stored baseline."
