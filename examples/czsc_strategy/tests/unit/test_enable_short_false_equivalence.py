"""A46 P7 — Default short-disabled full-engine equivalence regression.

Runs BacktestEngine end-to-end with ``enable_short=False`` (the unchanged
master default) on deterministic synthetic data.  The stored snapshot represents
the legacy baseline; any future change that alters the default path will produce
...
SNAPSHOT_PATH = Path(__file__).with_name("test_enable_short_false_equivalence.snapshot.json")
...
def _make_mock_signals(open_at_call: int = 8, close_at_call: int = 10):
    """Return a deterministic get_all_signals replacement that forces one long trade."""
...
def test_enable_short_false_full_engine_equivalence(synthetic_1m_bars, monkeypatch):
    """Default enable_short=False must reproduce the stored baseline exactly."""
    actual = {
        symbol: _run_symbol(
            monkeypatch,
    ...
    assert actual == baseline, "enable_short=False output differs from stored baseline."
