...
用户已将本目录原地归档，缠论择时统一使用 `D:\repo\czsc-timing-engine`。
以 [ARCHIVED.md](ARCHIVED.md) 为当前项目状态说明；下列规则和 HANDOFF 的历史阶段、
未完成项、下一棒说明仅供历史复核，不自动触发新开发或运行。
本次仅更新归档说明，保留最后代码版本，不发布新的策略版本。
新增缠论择时研究应转到指定项目并读取其当前指令；不得把旧实现默认为新项目的信号来源。
