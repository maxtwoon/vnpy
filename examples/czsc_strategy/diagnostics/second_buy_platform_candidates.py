...
from diagnostics.strategy_execution_experiment_matrix import SECOND_BUY_FILTERS, TRAILING_OVERRIDES  # noqa: E402
...
CANDIDATES = {
    "current_AP_A_ZN": {
        ...
        "exempt_chase_symbols": ["AP888", "A888"],
    },
    "AP_RB_A_ZN": {
    ...
    "RB_A_ZN": {
        "enable_2buy_symbols": ["RB888", "A888", "ZN888"],
        "exempt_chase_symbols": ["A888"],
    },
    "A_ZN_only": {
        "enable_2buy_symbols": ["A888", "ZN888"],
        "exempt_chase_symbols": ["A888"],
    },
    "AP_A_only": {
        "enable_2buy_symbols": ["AP888", "A888"],
        "exempt_chase_symbols": ["AP888", "A888"],
    },
    "A_only": {
        "enable_2buy_symbols": ["A888"],
        "exempt_chase_symbols": ["A888"],
    },
    "ZN_only": {
        "enable_2buy_symbols": ["ZN888"],
        "exempt_chase_symbols": [],
    },
}
...
def _patch_evaluator(candidate: dict[str, Any], multiplier: float, first_buy_gate: dict[str, Any] | None = None):
    ...
    def scenario_params(name: str, symbol: str) -> dict[str, Any]:
        ...
        params.update(copy.deepcopy(first_buy_gate or {}))
        params["enable_2buy_symbols"] = list(candidate["enable_2buy_symbols"])
        if symbol in set(candidate["exempt_chase_symbols"]):
            params["max_2buy_entry_vs_anchor_pct"] = None
        else:
            params["max_2buy_entry_vs_anchor_pct"] = SECOND_BUY_FILTERS["max_2buy_entry_vs_anchor_pct"]
        return params
...
def _evaluate_candidate(
    ...
    first_buy_gate: dict[str, Any] | None,
) -> dict[str, Any]:
    evaluator, original, scenario = _patch_evaluator(candidate, multiplier, first_buy_gate)
    try:
...
def build_report(
    ...
    candidates: list[str],
    symbol_set_names: list[str],
    first_buy_gate: dict[str, Any] | None = None,
) -> dict[str, Any]:
    rows = {}
    for name in candidates:
        rows[name] = _evaluate_candidate(
            db_path, name, CANDIDATES[name], multiplier, symbol_set_names, first_buy_gate
        )
    return {
        "multiplier": multiplier,
        "first_buy_gate": first_buy_gate or {},
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Scan second-buy eligibility candidates for platform robustness.")
    parser.add_argument("--db-path", type=Path, default=Path(SQLITE_DB_PATH))
    parser.add_argument("--multiplier", type=float, default=0.847)
    parser.add_argument("--candidates", nargs="+", default=["current_AP_A_ZN", "A_ZN_only", "A_only"])
    parser.add_argument("--sets", nargs="+", default=DEFAULT_SYMBOL_SET_NAMES, choices=sorted(SYMBOL_SETS))
    parser.add_argument("--first-buy-gate", choices=["none", "block_daily_down"], default="none")
    parser.add_argument("--out-json", type=Path, default=HERE / "second_buy_platform_candidates.json")
    parser.add_argument("--out-md", type=Path, default=HERE / "second_buy_platform_candidates.md")
    ...
    payload = build_report(args.db_path, args.multiplier, args.candidates, args.sets, gates[args.first_buy_gate])
