...
def build_daily_brief(summary: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "## 历史 DB 更新",
        "",
        f"- historical_db_update.status: `{_safe_get(summary, 'historical_db_update', 'status', default='skipped')}`",
        f"- historical_db_update.exit_code: `{_safe_get(summary, 'historical_db_update', 'exit_code', default='')}`",
        f"- historical_db_update.started_at: `{_safe_get(summary, 'historical_db_update', 'started_at', default='') or '无'}`",
        f"- historical_db_update.ended_at: `{_safe_get(summary, 'historical_db_update', 'ended_at', default='') or '无'}`",
        "",
        "## 账户污染监控",
        "",
