...
When `simnow_run_summary_YYYY-MM-DD.json` reports `automation_status=halt`, the wrapper must generate a read-only review pack:

- `simnow_risk_halt_review_YYYY-MM-DD.json`
- `simnow_risk_halt_review_YYYY-MM-DD.md`

The review pack must expose the authoritative automation status/action, safety snapshot, environment/replay readiness, threshold diagnostics, and risk-source breakdown needed for manual review. It must not connect to SimNow, send orders, or read private config.

Every applicable risk-halt review must also have a decision record:

- `simnow_risk_halt_decision_YYYY-MM-DD.json`
- `simnow_risk_halt_decision_YYYY-MM-DD.md`

The default decision record must be safe: `decision_status=pending_decision` and `next_formal_observation_allowed=false`. A decision is valid only when all of the following fields are complete:

- `decision_status=decided`
- `selected_decision` is one of the allowed decision options
- `operator_name` is non-empty
- `rationale` is non-empty
- `requires_observation_window_reset` is explicitly true or false
- `next_formal_observation_allowed=true`

`run_next_work.ps1 -LiveCapture` must block before any SimNow connection attempt when a prior `simnow_risk_halt_decision_*.json` remains pending, incomplete, invalid, or does not set `next_formal_observation_allowed=true`. `-Preflight` and `-PostProcessOnly` may still run so artifacts can be validated or repaired without starting a new live capture.
...
- `action_class`: machine-readable next-step category (`counts_for_20d`, `rerun_next_session`, `wait_for_data`, `investigate_infra`, `resolve_observation_gaps`, `manual_review_required`, `unknown`).
- `blocker_class`: machine-readable urgency bucket (`none`, `wait`, `review_now`).
...
The promotion decision report must also surface the structured blocker aggregates already present in JSON outputs:

- `blocking_action_counts`: grouped counts by `action_class`.
- `top_blocking_actions`: a tabular summary with `reason`, `days`, `action_class`, and `blocker_class`.
...
- 20-day observation progress (`ledger_summary`) when available, including `valid_observation_days`, `consecutive_valid_days`, `ready_to_expand`, `promotion_blockers`, `reason_counts`, `blocking_action_counts`, `latest_action`, `next_action`, and `next_action_class`.
- automation-platform status (`automation_status`), exit code (`automation_exit_code`), reason (`automation_reason`), recommended action (`automation_action`), and the machine-readable fields `automation_action_class` / `automation_blocker_class`.
