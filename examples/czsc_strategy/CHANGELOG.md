...
- 新增 `docs/design/A106_trend_type_signal.md`：按书第 75~79 页定义设计
  `{freq}_D1ZS_走势类型V260729` 完全分类信号（无中枢/盘整/上涨趋势/下跌趋势/中枢延伸），
  含四条设计决策（D1 中枢来源与已确认笔 / D2 依次抬降判据 / D3 zg-zd 无重叠判据 / D4 几何方向）
  与 AC1~AC7 可判定验收标准；新增只读信号，默认不接入任何门控，基线字节不变。
- `HANDOFF.md` 由 A32 superseded 占位重写为 A106（stage=design, owner=claude-cowork），
  附决策记录（编号选取、无 `handoff.py new` 子命令的替代路径）。
- 纯文档改动，不涉及代码与回测行为。
