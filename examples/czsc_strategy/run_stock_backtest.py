...
# -----------------------------------------------------------------------------
# 本文件（run_stock_backtest.py）是早期 A 股原型代码，已停止维护，未接入当前回测/测试路径；
...
#
# 已知偏差（仅披露，未修复）：本脚本的股票池（下方 STOCKS）按回测窗口“结束日”
# 静态选取（选股日期 2024-12-31，见 STOCKS 注释），构成幸存者偏差 / 前视
# （look-ahead）——股票须一直存续并满足筛选条件直到窗口结束日才会入选，而回测
# 起点远早于该日。
#
